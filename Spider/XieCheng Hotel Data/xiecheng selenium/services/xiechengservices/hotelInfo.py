...
class XiechenghotelService(object):
    ...
    def intohotel(self,Links):
        ...
        try:
            # 爬取酒店基本数据
            items = self.crawlhotelinfo(response)
            # 爬取酒店房间数据
            # itemss = self.crawlbedinfo(response)
        except:
            items = self.crawlhotelinfo2(response)
        # 存取酒店基本数据
    ...
    # 爬取酒店房间信息
    def crawlbedinfo(self,response):
        time.sleep(3)
        items = []
        itemDict = dict()
        try:
            bedname = response.xpath("//div[@class='htl_room_table J_roomTable']/table/tbody/tr[@data-disable='0']/td[1]/a[2]/text()").extract()[0]
        except:
            bedname = bedname

        buytype = response.xpath("//div[@class='htl_room_table J_roomTable']/table/tbody/tr[@data-disable='0']/td[@class='child_name']").extract()[0]
    ...
    # 爬取酒店基本数据（另一种情况）
    def crawlhotelinfo2(self,response):
            ...
            # 爬取周边设施
            around = itemData.xpath("//div[@class='hotel_info_comment']/div[@class='htl_info_table'][2]/table/tbody/tr")
            around_faciities = ""
            if len(around):
                for i in range(len(around)):
                    aroundhead = around[i].xpath("th/text()").extract()[0]
                    aroundbody = around[i].xpath("td/ul/li/text()").extract()
                    around_body = ""
                    for a in range(len(aroundbody)):
                        around_body += aroundbody[a]
                        around_body += ","
                    around1 = aroundhead + ":" + around_body
                    around_faciities = around_faciities  + around1 + ";"
            else:
                pass
            itemDict["around_facilities"] = around_faciities
            items.append(itemDict)
            return items
    ...
    # 爬取酒店基本数据
    def crawlhotelinfo(self,itemData):
            ...
            # 标题
            title = itemData.xpath("//div[@class='name']/h2[@class='cn_n']/text()").extract()[0]
            if len(title):
                itemDict["title"] = title
            else:
                itemDict["title"] = ""
            ...
            # 价格
            price = itemData.xpath("//div[@class='price_box']/div/p/span[@class='price']/text()").extract()
            if len(price):
                itemDict["price"] = price[0]
            else:
                itemDict["price"] = ""
            ...
            # 地址
            area = itemData.xpath("//div[@class='adress']/span/text()").extract()
            if (len(area)):
                areas = ""
                for i in range(0,3,1):
                    areas += area[i]
            ...
            # 有无wifi
            havewifi1 = itemData.xpath("//div[@class='icon_list']/i[@class='icons-facility32']")
            havewifi2 = itemData.xpath("//div[@class='icon_list']/i[@class='icons-facility32']")
            if (havewifi1 or havewifi2):
                itemDict["havawifi"] = "yes"
            else:
                itemDict["havawifi"] = "no"
            ...
            itemDict["service_facilities"] = ""
            itemDict["room_facilities"] = ""
            hotel_facilities = itemData.xpath("//div[@class='hotel_info_comment']/div[@id='J_htl_facilities']/table/tbody/tr")
            if len(hotel_facilities):
                for a in range(0,len(hotel_facilities)-1):
                    sheshiname = hotel_facilities[a].xpath("th/text()").extract()[0]
                    Hotel_facilities = hotel_facilities[a].xpath("td/ul/li/@title").extract()
